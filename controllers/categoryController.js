...
export const addCategory = asyncWrapper(
    async (req, res, next) => {
        ...
        if (!title || !description) {
            return next(appError.create("Title and description are required for category", 404, httpStatus.ERROR));
        }
...
export const updateCategory = asyncWrapper(
    async (req, res, next) => {
        const updatedCategory = await Category.findByIdAndUpdate(req.params.id, req.body, { returnDocument: 'after' })
        if (!updateCategory) {
            return next(appError.create("Category not found", 404, httpStatus.ERROR))
        }
        res.status(200).json({
